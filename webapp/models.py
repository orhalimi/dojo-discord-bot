...
class Room(BaseModel):
    ...
    # @admin.display(description='Super Mentor')
    # def get_supermentor(self):
    #     qet_members = Member.objects.filter(room = self.id)
    #     for m in qet_members:
    #         if m.role.name == "Super Mentor":
    #             return m
            
    @admin.display(description='Events')
    def get_events(self):
        events = Event.objects.filter(rooms = self.id).__len__()
        return events
...
class Event(BaseModel):
    ...
    rooms = models.ManyToManyField("webapp.Room")
